...
d + 10 * n은 프로덕션 룰을 통해 sytax trees를 그려보면 해당 구문이 Expression nonterminal symbol인걸 알 수 있다. 그리고 Mini-Triangle에서 모든 이진 연산이 동일 우선순위를 갖기 때문에 (d + 10) * n 으로 계산 된다는 것도 알 수 있다.
